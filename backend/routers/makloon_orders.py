...
async def create_order(payload: MakloonOrderCreate, request: Request) -> Dict[str, Any]:
    ...
    # FASE D (PS-03) — override yield WAJIB disertai alasan agar bisa diaudit.
    settings = await cs.get_settings(entity_id)
    if settings.get("require_yield_reason", True):
        for i, s in enumerate(data.get("steps") or [], start=1):
            if float(s.get("yield_factor") or 0) > 0 and not (s.get("yield_override_reason") or "").strip():
                raise HTTPException(
                    status_code=400,
                    detail=f"Langkah {i}: mengisi yield secara manual adalah OVERRIDE atas "
