                ...
                <tbody>
                  {filtered.map((r) => (
                    <tr key={`${r.customer_id}-${r.entity_id}`} data-testid={`sc-row-${r.customer_id}`}
                      ...
                      <td className="px-3 py-2 text-[#6B6B73]">{r.entity_id || "—"}</td>
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          {loading ? <div className="h-9 bg-[#F5F5F7] rounded animate-pulse mt-1" /> : orders.length === 0 ? (
            ...
            <select data-testid="sc-redeem-order" className="field w-full mt-1" value={orderId} onChange={(e) => setOrderId(e.target.value)}>
              {orders.map((o) => (
                <option key={o.order_id} value={o.order_id}>{o.number} — outstanding {formatCurrency(o.outstanding)}</option>
              ))}
            </select>
          )}
        </div>
