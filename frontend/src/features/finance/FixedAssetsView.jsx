          ...
          <div className="modal-card" style={{ maxWidth: 560, width: "95vw" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid gap-3 mt-3">
              <div>
                ...
                <select data-testid="asset-transfer-to" className="field" value={trTo}
                  onChange={(e) => setTrTo(e.target.value)}>
                  <option value="">— Pilih badan usaha —</option>
                  {(entities || []).filter((e) => e.id !== transferFor.entity_id
                    && (e.status || "active") === "active").map((e) => (
                    <option key={e.id} value={e.id}>{e.short_name || e.legal_name || e.id}</option>
                  ))}
                </select>
              </div>
              <div>
