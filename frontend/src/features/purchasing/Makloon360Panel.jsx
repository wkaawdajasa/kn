...
export default function Makloon360Panel({ makloonId, currentUser, onBack, onEdit, onError }) {
  ...
  function openOrder(o) {
    setRecord({
      ...
      items: o.steps || [],
      itemColumns: [
        { label: "Proses", render: (s) => PROCESS_LABELS[s.process_type] || s.process_type || "—" },
        ...
        <div className="section-head">
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap gap-1.5">
              {(data.process_types || []).map((p) => <span key={p} className="rounded-full bg-[#EAF2FF] px-2 py-0.5 text-[10.5px] font-semibold text-[#0058CC]">{PROCESS_LABELS[p] || p}</span>)}
            </div>
          </div>
    ...
    <div className="divide-y divide-[#EFF0F2] max-h-[460px] overflow-y-auto" data-testid="makloon-360-list-recipes">
      {rows.map((r) => (
        <div key={r.id} className="flex items-center justify-between py-2 text-[11.5px]">
          <div className="min-w-0"><p className="truncate font-semibold">{r.name}</p><p className="text-[10px] text-[#6B6B73]">{PROCESS_LABELS[r.process_type] || r.process_type} · yield {r.yield_factor}</p></div>
