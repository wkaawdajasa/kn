...
const PROC_FILTER = [{ value: "", label: "Semua Proses" }, ...Object.entries(PROCESS_LABELS).map(([v, label]) => ({ value: v, label }))];
          ...
          <div className="divide-y divide-[#EFF0F2] max-h-[600px] overflow-y-auto">
            {filtered.map((r) => (
              <div key={r.id} data-testid={`recipe-row-${r.id}`} className="grid grid-cols-[1.4fr_1.6fr_1fr_150px_100px] items-center px-3 py-2.5 hover:bg-[#FAFBFC]">
                <div className="min-w-0"><p className="text-[12px] font-semibold truncate">{r.name}</p><span className="rounded bg-[#F3E9FA] px-1.5 text-[10px] font-bold text-[#6B219A]">{PROCESS_LABELS[r.process_type] || r.process_type}</span></div>
