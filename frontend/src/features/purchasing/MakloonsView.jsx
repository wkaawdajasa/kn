            ...
            {filtered.map((m) => (
              <div key={m.id} data-testid={`makloon-row-${m.id}`} className="grid grid-cols-[90px_1.5fr_1.1fr_130px_80px_110px] items-center px-3 py-2.5 hover:bg-[#FAFBFC]">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10.5px] text-[#6B6B73] truncate flex items-center gap-1"><EntityBadge entityId={m.entity_id} /><span className="truncate">{(m.process_types || []).map((p) => PROCESS_LABELS[p] || p).join(", ") || "—"}</span></p>
                </div>
