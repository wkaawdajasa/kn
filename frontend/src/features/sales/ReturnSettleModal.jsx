      ...
      <div className="modal-card" style={{ maxWidth: 640 }}>
        ...
        {movesStock && (
          <div className="rounded-md border border-[#E4ECF7] bg-[#F6FAFF] p-2.5 mb-3" data-testid="settle-destination">
            ...
            <select className="field w-full" data-testid="settle-return-warehouse"
              value={returnWarehouseId} onChange={(e) => setReturnWarehouseId(e.target.value)}>
              <option value="">Default cerdas (gudang pengiriman SO)</option>
              {warehouses.map((w) => (
                <option key={w.id} value={w.id}>{w.name || w.code || w.id}{w.city ? ` — ${w.city}` : ""}</option>
              ))}
            </select>
        ...
        {outcome === "refund" && (
          <div className="rounded-md border border-[#E7E1F5] bg-[#F8F5FE] p-2.5 mb-3" data-testid="settle-refund-account-box">
            ...
            <select className="field w-full" data-testid="settle-refund-account"
              value={refundAccount} onChange={(e) => setRefundAccount(e.target.value)}>
              <option value="">Default — 1-1100 Kas Besar / Bank</option>
              {cashAccounts.map((a) => (
