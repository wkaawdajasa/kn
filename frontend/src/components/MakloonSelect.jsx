 ...
 * Dipakai di form Resep Proses & (nanti) Order Makloon. onSelect(makloon).
 */
...
export const PROCESS_LABELS = {
  tenun: "Tenun", celup: "Celup", finishing: "Finishing", printing: "Printing", lainnya: "Lainnya",
};
          ...
          {loading ? <div className="py-8 text-center text-[12px] text-[#6B6B73]">Memuat…</div>
           ...
           : filtered.length === 0 ? (
            <div data-testid="makloon-select-empty" className="py-8 text-center text-[12px] text-[#8E8E93]">
              Tidak ada makloon{processType ? ` untuk proses ${PROCESS_LABELS[processType] || processType}` : ""}.
            ...
            <button key={m.id} data-testid={`makloon-select-item-${m.id}`} onClick={() => onPick(m)}
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-[10.5px] text-[#6B6B73]">{m.city || "—"} · {(m.process_types || []).map((p) => PROCESS_LABELS[p] || p).join(", ") || "—"}</p>
              </div>
  ...
  return (
    <div data-testid="makloon-quick-create" className="border-b border-[#EFF0F2] bg-[#FAFBFC] px-4 py-3 space-y-2">
      ...
      <div className="flex flex-wrap gap-1.5">
        {Object.entries(PROCESS_LABELS).map(([k, lbl]) => (
