...
DOOR_EXEMPT: Dict[str, str] = {
    ...
    "design_gallery.py::/api/design-gallery/{gallery_id}/approve":
        "Pengesahan DRAF desain: `draft` adalah keadaan bekerja milik pembuatnya (belum "
        "ada langkah 'ajukan'), jadi menghitungnya berarti menyebut pekerjaan orang lain "
        "sebagai antrean keputusan.",
    ...
    "hr_payroll.py::/api/hr/payroll/runs/{run_id}/approve":
        "UTANG ALUR: pengesahan payroll bekerja dari status `draft` — draf yang masih "
        "dikerjakan HR tak bisa dibedakan dari yang siap disahkan. Menambah langkah "
        "'ajukan' dulu, baru antreannya bisa jujur.",
    "payment_variance.py::/api/payment-variances/receipt/{receipt_id}/decide":
        "UTANG ALUR: dokumen keputusan (`payment_variance_decisions`) baru lahir SAAT "
        "diputuskan; keadaan menunggunya hidup sebagai selisih pada kwitansi "
        "(`ar_receipts`) tanpa status tersendiri, jadi belum bisa dihitung tanpa menebak.",
    ...
    "work_desks.py::/api/sales-orders/{order_id}/verify":
        "UTANG ALUR: verifikasi ADMINISTRATIF Admin Sales menempel sebagai "
        "`sales_orders.verification` (bukan status dokumen), antreannya Meja Admin Sales. "
        "Bila ingin ikut dihitung, butuh baris antrean sendiri yang tidak dobel dengan "
        "baris `sales_order`.",
}
...
DATA_EXEMPT: Dict[Tuple[str, str], str] = {
    ...
    ("design_gallery", "draft"):
        "Draf desain milik pembuatnya (lihat pembebasan pintu design-gallery).",
    ("hr_payroll_runs", "draft"):
        "Draf payroll (lihat UTANG ALUR pada pembebasan pintu payroll).",
