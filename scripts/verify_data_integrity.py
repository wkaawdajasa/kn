...
async def layer_makloon_invariants(db):
    ...
    """
    import re as _re
    print(f"\n{C}{B}L4-MKO — Invarian Makloon Rantai Proses (Fase D · D-05/D-07/D-09){X}")
    ...
    for o in orders:
        for s in o.get("steps", []):
            if s.get("status") != "received":
                continue
            calc = (float(s.get("material_value") or 0) + float(s.get("service_value") or 0)
                    - float(s.get("byproduct_value") or 0))
    ...
    if drift:
        ...
        line("FAIL", R, f"INV-MKO-04: {len(drift)} langkah nilai output tidak rekonsiliasi "
                        "(bahan+jasa−sisa ≠ output)", str(drift[:5]))
    else:
        results["pass"] += 1
        line("PASS", G, "INV-MKO-04: nilai output tiap langkah = bahan + jasa − barang sisa "
                        "(WIP di-clear penuh)")
